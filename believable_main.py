...
if __name__ == "__main__":
    ...
    POIs = {}
    USER = {}
    observe_user = be.find_observe_user()
    ...
    with tqdm(total=len(ou_charges), ncols=100) as bar:
        for ou_charge in ou_charges:
            ...
            users_pi = [pool.apply_async(opsm.payment_stage, (USER, POIs,
                                                              user, winner, False, budget, sequence)) for user in winner["winner"]]
            ...
            storege.storage_all(winner['winner'], POIs, 'document/believable_result/winner/winner_POIs/', ou_charge)
            storege.storage_observe_user('document/believable_result/winner/observe_USER/', observe_user, ou_charge)
            bar.update(1)
