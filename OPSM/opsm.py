...
def allocation_stage(USER: dict, POIs: dict, log_display=False):
    winner = []

    winner_num = 0
    max_dv_user = opsm_fun.find_max_value_user(USER, winner, POIs)  # dv 为 density value 的缩写
    while (max_dv_user.id != 0) and (max_dv_user.charge <= opsm_fun.threshold(winner, max_dv_user, POIs)):
        winner.append(max_dv_user)
        max_dv_user.allocated = 1
        winner_num += 1
        print(f"Round {winner_num + 1}")
        max_dv_user = opsm_fun.find_max_value_user(USER, winner, POIs)
...
def payment_stage(USER: dict, POIs: dict, user: Person, winner: list, log_display=False):
    ...
    while (max_user.charge < opsm_fun.threshold(winner_primer, max_user, POIs)) and (max_user.id != 0):
        min = opsm_fun.min_pi(winner_primer, user, max_user, POIs)
        if log_display:
