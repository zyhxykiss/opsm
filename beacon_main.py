...
# from BEACON import data_format
...
#         USER = data_format.USER_format(os.path.join(path, user_file))
#
...
#         winner = beacon.allocation_stage(USER, POIs, True)
#
#         print("-----------------------------Payment Stage-------------------------------")
#         pool = mp.Pool(processes=param["processes"])
#         users_pi = [pool.apply_async(beacon.payment_stage, (USER, POIs, user, winner, True)) for user in winner]
#
#         total_payment = 0
#         i = 0
#         for user in winner:
...
#
#         storege.storage_all(winner, POIs, 'document/beacon_result/USER_CHANGE', int(user_file.split('.')[0]))
#
#     dt = datetime.now()
...
if __name__ == "__main__":
    ...
    with tqdm(total=len(os.listdir(path)), ncols=100) as bar:
        for user_file in os.listdir(path):
            ...
            print("-----------------------------Payment Stage-------------------------------")
            pool = mp.Pool(processes=param["processes"])
            users_pi = [pool.apply_async(beacon.payment_stage, (USER, POIs, user, winner, False)) for user in winner["winner"]]
            ...
            storege.storage_all(winner["winner"], POIs, 'document/beacon_result_0.8/USER_CHANGE/', int(user_file.split('.')[0]))
            bar.update(1)
    dt = datetime.now()
    print(dt.strftime("完成时间：%Y年%m月%d日 %H:%M:%S"))
