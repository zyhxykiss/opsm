...
from Reviewer import reviewer_fun
import numpy as np
import pandas as pd
...
def allocation_stage(USER: dict, POIs: dict, log_display=False, budget=param["budget"], seq=None):
    ...
    winner_num = 0
    max_dv_user = reviewer_fun.find_max_value_user(USER, winner, POIs, True, seq)  # dv 为 density value 的缩写
    while (max_dv_user.id != 0) and (max_dv_user.charge <= reviewer_fun.threshold(winner, max_dv_user, POIs, budget, seq)):
        winner["winner"].append(max_dv_user)
        max_dv_user.allocated = 1
        winner_num += 1
        # print(f"Round {winner_num + 1}")
        max_dv_user = reviewer_fun.find_max_value_user(USER, winner, POIs, True, seq)
...
def payment_stage(USER: dict, POIs: dict, user: Person, winner: dict, log_display=False, budget=param['budget'], seq=None):
    ...
    max_user = reviewer_fun.find_max_value_user(user_prime, winner_primer, POIs, seq=seq)

    while (max_user.charge < reviewer_fun.threshold(winner_primer, max_user, POIs, budget, seq)) and (max_user.id != 0):
        min = reviewer_fun.min_pi(winner_primer, user, max_user, POIs, budget, seq)
        if log_display:
